...
void Game::tendToItems() {
    ...
    // First loop: Collect seeds/plants/animals to process without modifying the inventory yet
    for (auto it = inventory.getItems().begin(); it != inventory.getItems().end(); ++it) {
        Item* item = *it;

        // Check if the item is a Seed
        if (Seed* seed = dynamic_cast<Seed*>(item)) {
            string plantType = seed->getPlantType(); // Get the type of plant the seed grows into

            if (!plantType.empty()) {
                // Check if the plant already exists in the inventory
                auto plantIt = std::find_if(inventory.getItems().begin(), inventory.getItems().end(),
                ...
                if (plantIt != inventory.getItems().end()) {
                    ...
                    (*plantIt)->setItemCount((*plantIt)->getItemCount() + seed->getItemCount());
                } else {
                    // Check if the plant already exists in the itemsToAdd vector
                    auto newPlantIt = std::find_if(itemsToAdd.begin(), itemsToAdd.end(),
                        [&plantType](Item* i) { return i->getName() == plantType; });

                    if (newPlantIt != itemsToAdd.end()) {
                        // If the plant exists in itemsToAdd, increment its count
                        (*newPlantIt)->setItemCount((*newPlantIt)->getItemCount() + seed->getItemCount());
                    } else {
                        // If the plant doesn't exist in either, grow the seed into a plant and add it
                        Plant* newPlantInstance = seed->grow();                     // Grow the seed into a new plant
                        if (newPlantInstance) {
                            newPlantInstance->setItemCount(seed->getItemCount());   // Set the correct count
                            itemsToAdd.push_back(newPlantInstance);                 // Queue plant to be added
                        }
                    }
                }
        ...
        // Check if the item is a YoungAnimal (similar logic can be applied here)
            ...
            string animalType = youngAnimal->getGrownAnimalType(); // Get the type of animal being grown

            // Check if the animal already exists in the inventory
            auto animalIt = std::find_if(inventory.getItems().begin(), inventory.getItems().end(),
                [&animalType](Item* i) { return i->getName() == animalType; });

            if (animalIt != inventory.getItems().end()) {
                (*animalIt)->setItemCount((*animalIt)->getItemCount() + youngAnimal->getItemCount());
            } else {
                // Check if the grown animal already exists in itemsToAdd vector
                auto newAnimalIt = std::find_if(itemsToAdd.begin(), itemsToAdd.end(),
                    [&animalType](Item* i) { return i->getName() == animalType; });

                if (newAnimalIt != itemsToAdd.end()) {
                    (*newAnimalIt)->setItemCount((*newAnimalIt)->getItemCount() + youngAnimal->getItemCount());
                } else {
                    // If the grown animal doesn't exist, grow the young animal into a grown one
                    GrownAnimal* grownAnimal = youngAnimal->grow();             // Grow the young animal
                    if (grownAnimal) {
                        grownAnimal->setItemCount(youngAnimal->getItemCount()); // Set the correct count
                        itemsToAdd.push_back(grownAnimal);                      // Queue grown animal to be added
                    }
                }
            }
    ...
    if (seedsGrown) {
        cout << "Your seeds grew into plants and are ready to be harvested and sold.\n";
    }
...
void Game::sellItems() {
    ...
    for (auto item : itemsToRemove) {
        inventory.removeItem(item);  // This handles deletion and removal from inventory
    }
